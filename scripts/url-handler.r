...
REBOL [
  ...
  Date: 16-Aug-2003
  History: [
             06-Nov-2016 {Permit relative URLs to start with //}
...
url-handler-object: context [
  ...
  init: func [ /local relative? ] [
    ...
    relative?: find/part url "//" 2

    if all [relative? "" = protocol] [
      make error! join "Lacking protocol information: " url
    ]
    ...
    either relative? [
      ...
      ; if rest starts with a double slash, we are to keep only the protocol
      if find/part rest "//" 2 [
      	 ; host is to be replaced
	 rest: at rest 3
	 phost: first parse rest "/"
	 dismantle-host
      ...
      ; if rest starts with a single slash, the whole path is to be replaced, else simply attach:
