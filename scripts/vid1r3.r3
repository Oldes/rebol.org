REBOL [
	...
	History: [
		...
		0.3.16 [24-04-2016 "fixed window title reset in show"]
		0.3.17 [20-05-2017 "fixed resetting text in show func"]
		0.3.18 [28-01-2018 "Added first native, extended system/standard with face, fixed reduce path value in to-r3-draw-block"]
		0.3.19 [16-05-2020 "fixed luma function"]
		0.3.20 [17-04-2022 "fixed to-integer for r3-view_3.0.99.3.3"]
	]
	library: [
		level: 'advanced
		platform: 'windows
		type: 'module
		domain: [sdk gui vid]
		tested-under: [Saphir-View 2.101.0.3.1 Atronix-View 3.0.99.3.3]
		support: none
...
; natives (R3-backward)
	foreach word [
		...
		to-word shift wait to-string pick event? third poke first make func does has find
		][
			...
			either all[
				...
				case [
					find system/view/screen-face/pane face [ ; a window?
						...
						;print ["face changed " face/text face/gob/text face/gob/parent/text face/gob/parent/offset face/gob/parent/size face/offset face/size]
						...
						if get in subface/gob/data 'gob-text [ subface/gob/data/gob-text/text: ""] ; reset this
	...
	to-integer: func [value][to integer! to decimal! any [:value 0]]
	...
	first: func [
		...
		series [series! pair! event! money! date! object! port! time! tuple! any-function! library! struct! event! gob!]
		][
		if object? :series [return head insert words-of :series 'self]
		native-first series
	]
	second: func [
	...
	find: func [
	    ...
	    series [series! gob! port! bitset! typeset! object! none! issue!]
	    value [any-type!]
		...
		either all [issue? series issue? value] [
			attempt [to issue! find form to word! series form to word! value]
			][
			apply :native-find [series value part length only case any with wild skip size last reverse tail match]
		]
	]
	
	{
	make: func ["Constructs or allocates the specified datatype."
		type [any-type!] "The datatype or an example value"
		spec [any-type!] "Attributes or size of the new value (modified)"
		;'body [block! word! set-word! unset!]
		][
		either function! = type [native-make function! copy/deep reduce [spec :body]][native-make type spec]
	]
	
		;make: func [type [any-type!] spec [any-type!] 'body [any-type!]][either function! = type [native-make function! copy/deep reduce [spec :body]][native-make function! type spec]]
	func: native-make function! [[
	    "Defines a user function with given spec and body."
	    spec [block!] {Help string (opt) followed by arg words (and opt type and string)}
	    body [block!] "The body block of the function"
		][
	    native-make function! copy/deep reduce [spec body]
	]]
	does: native-make function! [[
	    {A shortcut to define a function that has no arguments or locals.}
	    body [block!] "The body block of the function"
		][
	    native-make function! copy/deep reduce [[] body]
	]]
	has: native-make function! [[
	    {A shortcut to define a function that has local variables but no arguments.}
	    vars [block!] "List of words that are local to the function"
	    body [block!] "The body block of the function"
		][
	    native-make function! reduce [head insert copy/deep vars /local copy/deep body]
	]]
	}
	; I am not able to patch these natives:
		; make, to, decompress, do, throw
; mezz-gfx
	rgb-to-hsv: func [
...
; gfx-funcs ; make_gobs_from_face
	...
	luma: func [image [image!] depth [integer!]
		/local pos pixel color
		][
		depth: min max -255 depth 255 ; clip value
		if depth = 0 [return image]
		repeat pos image [
			pixel: first pos
			; this adjusts brightness ...
			color: ((min max 0 depth + pixel/1 255) * 1.0.0) + ((min max 0 depth + pixel/2 255) * 0.1.0) + ((min max 0 depth + pixel/3 255) * 0.0.1)
			change/only pos color
		]
		image
	]
	...
	to-r3-draw-block: func [[catch] block [block!] /local draw-rule draw-words at out value value2 values beg fin stroke-color dash-color fon][
		...
		substitute: does [change/part/only beg head out fin clear out]
		out: copy []
		values: copy []
		forall block [
			...
			if path? value [change block get value]
		] ; reduce block
		pos: head block
	...
	draw-block-from-effect: func [face [object!]
		...
		block: copy/deep compose [reduce (face/effect)]
		forall block [
			set/any 'value pick block 1
			if not any [
					scalar? value
		...
		effect-rule: [
			...
			| 'extend beg: 0 2 pair! fin: (insert/part clear values beg 2)
				(
				...
				image: ctx-image-effects/extend image size offset thick
				)
			| 'fit (
				size: face/size - any [attempt [face/edge/size * 2] 0x0]
				image2: copy image
	...
	call-iters: func [
		...
		index: 1
		while [faces: face/pane face index ] [
			index: index + 1
	...
	make_gobs_from_face: func [{Recursively create gobs from given face}
		...
		; create gobs
			...
			; image
				...
				gobs/gob-image: gob
				append gobbg gob
				if image? face/image [gobbg/color: none] ; in R2 face's image removes face's "background" color ?
			; effect ; done inside image ; FIXME: use "effect" gob instead !!
			; draw
			; WARNING: translate R2 draw block to R3 draw block !! :(
			if attempt [face/effect/draw] [
	...
	update_gobs_from_face: func [{Recursively update gobs from given face}
		...
		; update gobs
			...
			if gobim: face/gob/data/gob-image [
				...
				gobim/size: size ; restore because image could have changed it (!)
				if image? face/image [gobbg/color: none] ; in R2 face's image removes face's "background" color
			]
			; effect
			; draw
...
; gfx-object
	...
	protect 'face
	extend system/standard 'face face
